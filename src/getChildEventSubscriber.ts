...
export default function getChildEventSubscriber(
  ...
  const getChildSubscribers = (evtName: NavigationEventType) => {
    switch (evtName) {
      case 'action':
  ...
  const emit = (type: NavigationEventType, payload: NavigationEventPayload) => {
  ...
  const upstreamSubscribers = upstreamEvents.map(eventName =>
    addListener(eventName, payload => {
      ...
      const childPayload: NavigationEventPayload = {
        action,
        context: `${key}:${action.type}_${payload.context || 'Root'}`,
        state: newRoute as NavigationState,
        lastState: lastRoute as NavigationState,
        type: eventName,
      };
  ...
  return {
    ...
    emit(eventName: NavigationEventType, payload: NavigationEventPayload) {
      if (eventName !== 'refocus') {
        console.error(
